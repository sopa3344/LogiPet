...
모찌는 생산성을 평가하는 도구가 아니라, 사용자의 마우스 활동에 반응하며
...
- 마우스를 움직이면 모찌가 자리를 벗어나지 않고 함께 걷거나 달립니다.
- 커서가 모찌의 왼쪽·오른쪽으로 이동하면 그 방향을 바라봅니다.
- 잠시 입력이 없으면 기다리기 → 앉기 → 휴식 → 잠들기로 자연스럽게 바뀝니다.
- 모찌를 왼쪽 클릭하면 현재 시각과 오늘 활동에 맞는 대화를 합니다.
- 모찌를 오른쪽 클릭하면 간식, 물, 하이파이브, 스트레칭, 잠깐 놀기 메뉴가 열립니다.
